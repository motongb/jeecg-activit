...
//            message.setFrom(emailFrom);
    ...
    public List<ProcessNodeVo> getNextNode(String procDefId, String currActId, String procInsId) {
        // 根据流程实例id获取业务表单数据
        ...
        List<ProcessNodeVo> nodeVos = new ArrayList<>();
        // 当前执行节点id
        ...
        for (ActivityImpl activityImpl : activitiList) {
            if (activityImpl.getId().equals(currActId)) {
                ...
                if ("userTask".equals(type)) {
                    // 用户任务节点
                    nodeVos.add(parseNodeVo((ActivityImpl) pvmActivity, actBusiness));
...
//                    node.setType(ActivitiConstant.NODE_TYPE_EG);
    ...
    public ProcessNodeVo parseNodeVo(ActivityImpl pvmActivity, ActBusiness actBusiness) {
        ProcessNodeVo node = new ProcessNodeVo();
        node.setType(ActivitiConstant.NODE_TYPE_TASK);
